...
```bash
VideoProcessing$ sam build --use-container
```
...
```bash
VideoProcessing$ sam logs -n StockCheckerFunction --stack-name VideoProcessing --tail
```
...
```bash
VideoProcessing$ pip install -r tests/requirements.txt --user
# unit test
VideoProcessing$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
VideoProcessing$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name VideoProcessing
```
